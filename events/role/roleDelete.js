    ...
    if (await client.guild.findById(role.guild.id)) {
      ...
      if (guild.logs.role) {
        if (await guild.logs.role.toggle === "true") {
          ...
          const logs = await role.guild.fetchAuditLogs({ limit: 1, type: Discord.AuditLogEvent.RoleDelete, }).catch(console.error);
          if (logs) {
            ...
            if (channell) {
              const exampleEmbed = new Discord.EmbedBuilder()
                ...
                .setTitle(`Role Deleted By ${executor.username}`)
                .addFields(
      ...
      if (await guild.autorestore.roles === "true") {
        ...
        if (logs) {
          const log = logs.entries.find(l => l.action === Discord.AuditLogEvent.RoleDelete);
          const executor = log.executor;
          if (executor !== owner.user && executor.id !== client.user.id && !guild.trust.includes(executor.id)) {
            role.guild.roles.create(role)
          }
        }
      }
      ...
      if (guild.protection.antihack == "true") {
        ...
        const logsFinds = await role.guild.fetchAuditLogs({ limit: 1, type: Discord.AuditLogEvent.RoleDelete, }).catch(console.error);
        ...
        if (executorFind !== owner.user && executorFind.id !== client.user.id && !guild.trust.includes(executorFind.id)) {
          await role.guild.fetchAuditLogs().then(logs => {
            let actions = logs.entries
              .filter(e => e.action === Discord.AuditLogEvent.RoleDelete && e.executor.id === executorFind.id && (e.createdAt - Date.now()) < 86400000)
              ...
              if (guild.action.roles === "roles") {
                ...
                if (guild.notification && guild.notification === "true") {
                  const exampleEmbed = new Discord.EmbedBuilder()
                    ...
                    .addFields(
                      { name: `**Action :**`, value: `[** RoleDelete Limit **]`, inline: true },
